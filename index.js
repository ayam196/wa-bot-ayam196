...
const { help } = require('./Lisa/help')
const { bahasa } = require('./Lisa/bahasa')
const { donasi } = require('./Lisa/donasi')
const { limitend, limitcount } = require('./Lisa/limit')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ALIA\n'
            + 'ORG:Creator LISA;\n'
            + 'TEL;type=CELL;type=VOICE;waid=62085892910131:+62085892910131\n'
            + 'END:VCARD'

prefix = "_"
name = "~ L I S A | B O T"
rdaftar = "TERIMA KASIH TELAH DAFTAR😁"
rmenu = "HAI TEMEN LISA BOT👋"
limitt = 50
ban = []
userpremium = ["62085892910131@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	fs.existsSync('./Lisa.json') && client.loadAuthInfo('./Lisa.json')
	...
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Liza.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```List Banned``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```List User Premium L I S A B O T``` :\n'
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limitend(pushname2))
				    ...
				    break 
				case 'owner':
                 client.sendMessage(from, {displayname: "Lisa", vcard: vcard}, MessageType.contact, { quoted: mek})
...
//akhir kreator
		    ...
		    case 'darkjokes':
				...
				if (!isUser) return reply(mess.only.userB)
				reply(mess.wait)
				 data = fs.readFileSync('./Lisa/drak.js');
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 LISA BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 LISA BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat LISABOT\`\`\``)
					break
                                case 'bcgc':
					client.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *LISA_BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak *${pushname2}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[LISA_BOT]','red'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
